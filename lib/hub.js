...
exports.publish= function(channel, message){
    ...
    if(index>0){
        var baseIndex = pathPart.substring(0,index).lastIndexOf("/");
        var lastPart = channel.substring(index + 1);
        if(baseIndex<0){
            notifyAll("*/" + lastPart);
        }else{
            var basePart = pathPart.substring(0, baseIndex);
            notifyAll(basePart + "/*/" + lastPart);
        }
    }
	channel = pathPart;
	notifyAll(channel + "*");
